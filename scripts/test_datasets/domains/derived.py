...
def write_docs(config: Config, counts: dict[str, int]) -> None:
    ...
    lines = [
        "# Smartjoin Derived Regression Datasets",
        "",
        "Generated by `scripts/test_datasets/domains/derived.py`.",
    ...
    lines.extend(
        [
            "",
            "## Cases",
            "- prefix swap: `prefix_swap_events.product_key -> prefix_swap_dim.product_id`",
            "- strip non-alnum: `strip_non_alnum_events.invoice_ref -> strip_non_alnum_dim.invoice_id`",
            (
                "- remove prefix numeric: "
                "`remove_prefix_numeric_events.payment_key -> remove_prefix_numeric_dim.payment_id`"
            ),
            "- ambiguous collision guard: `ambiguous_events.product_key` competing against 3 candidate IDs",
            "- date-like guard: `order_dates.order_date -> calendar_dates.calendar_date` should stay direct",
            "- wide-budget stress: many derived-like candidate columns in `wide_budget_events`",
            "",
            "## Notes",
            "- generation only: no analysis, no evaluation, no debug-site output",
            "- deterministic output controlled by seed and profile",
        ]
    )
