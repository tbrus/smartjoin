"""Generate deterministic, join-heavy test datasets for Alchemia."""
...
def parse_args() -> Config:
    ...
    parser = argparse.ArgumentParser(
        description="Generate deterministic relational test data for Alchemia."
    )
    ...
    out_dir = args.out_dir or (Path("perf_data") / "datasets" / f"alchemia_{args.profile}")
    return Config(
        out_dir=out_dir,
        seed=args.seed,
...
def write_dataset_docs(config: Config, row_counts: dict[str, int]) -> None:
    ...
    manifest = {
        "generator": "scripts/generate_alchemia_testdata.py",
        "config": config_dict,
    ...
    lines = [
        "# Alchemia Performance Test Data",
        "",
        f"Generated by `scripts/generate_alchemia_testdata.py` with seed `{config.seed}`.",
